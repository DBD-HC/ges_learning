...
class FrameModel(nn.Module):
    ...
    def forward(self, rai, data_len):
        ...
        conv2d_feats = None
        if self.multistream:
            range_feats = self.range_conv(rai, batch_size=batch_size, padding_len=padded_len)
            angel_feats = self.angel_conv(rai, batch_size=batch_size, padding_len=padded_len)
        else:
            rai = self.spatial_conv(rai)
            rai = rai.view(rai.size(0), -1)
            rai = self.dp(rai)
            if self.need_diff:
                conv2d_feats = self.diff(rai, batch_size, padded_len)
            else:
...
class DifferentialNetwork(nn.Module):
    def __init__(self, in_size, out_size):
        ...
        self.linear_sp2 = nn.Sequential(
            nn.Linear(out_size // 4, out_size // 2, bias=True),
            nn.Hardswish(),
