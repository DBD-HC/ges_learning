...
time_frequency_data_root = '/root/autodl-tmp/dataset/mmWave_cd_tf'
...
class MCDDataSplitter(DataSpliter):
    def __init__(self, data_path=rai_data_root, is_multi_negative=False):
        ...
        super(MCDDataSplitter, self).__init__(data_path=data_path,
                                              domain_num=(5, 25, len(self.envs), len(self.locations)))
...
class MCDDataset(Dataset):
    def __init__(self, file_names, labels, transform=None, pre_processing=pre_processing, data_root=None, data_type=0, is_multi_negative=False):
        ...
        # time_freq
        elif data_type == data_type_map['TIME_FREQUENCY_IMAGE']:
            self.data_root = time_frequency_data_root
        # complex rdi
...
if __name__ == '__main__':
    ...
    acts = gestures + negative_samples
    envs = ['e1', 'e2', 'e3', 'e4', 'e5', 'e6']
    sparticipants = ['u1', 'u2', 'u3', 'u4', 'u5', 'u6', 'u7', 'u8', 'u9', 'u10', 'u11', 'u12', 'u13',
                         'u14', 'u15', 'u16', 'u17', 'u18', 'u19', 'u20', 'u21', 'u22', 'u23', 'u24', 'u25']
    locations = ['p1', 'p2', 'p3', 'p4', 'p5']

    vis = visdom.Visdom(env='aug', port=6006)
    d = np.load(os.path.join(complex_rdi_data_root, 'y_Clockwise_e3_u8_p5_s1.npy'))
    new_d = crop_rdi(d)
    d = np.sqrt(d[:, 0::2] ** 2 + d[:, 1::2] ** 2)
    d = np.mean(d, axis=1)
    for f_i, frame in enumerate(d):
        vis.heatmap(frame, win='origin', opts={'title':'origin'})
        vis.heatmap(new_d[f_i], win='new', opts={'title':'new'})
        time.sleep(5)
